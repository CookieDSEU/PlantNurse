package com.plantnurse.plantnurse.utils;

/**
 * Created by Cookie_D on 2016/8/11.
 */
public class Constants {
    public static final String WEATHER_KEY = "bec962b5eb6d4cea866512ee57a6c1df";// 和风天气 key
    public static final String WEATHER_API_URL = "https://api.heweather.com/x3/weather";
    public static final String SIGNIN_URL = "http://www.plantnurse.top/appops/login.php";
    public static final String SIGNUP_URL = "http://www.plantnurse.top/appops/signup.php";
    public static final String AVATAR_URL = "http://www.plantnurse.top/avatar/avatar.php";
    public static final String AVATAR_UPLOAD_URL = "http://www.plantnurse.top/avatar/upload.php";
    public static final String PLANT_UPLOAD_URL = "http://www.plantnurse.top/myplant/upload.php";
    public static final String CHECKVERSION_URL = "http://www.plantnurse.top/appops/checkversion.php";
    public static final String CHANGEINFO_URL = "http://www.plantnurse.top/appops/changeinfo.php";
    public static final String REPORT_URL = "https://www.sojump.hk/jq/9513426.aspx";
    public static final String GETINDEX_URL = "http://www.plantnurse.top/appops/getindex.php";
    public static final String PLANTICON_URL = "http://www.plantnurse.top/planticon/planticon.php?id=";
    public static final String PLANTPIC_URL = "http://www.plantnurse.top/plantpic/plantpic.php?id=";
    public static final String MYPLANTPIC_URL = "http://www.plantnurse.top/myplant/myplantpic.php?id=";
    public static final String PLANTINFO_URL = "http://www.plantnurse.top/appops/getplantinfo.php";
    public static final String RESETPSD_URL = "http://www.plantnurse.top/appops/resetpwd.php";
    public static final String NEWSTAR_URL = "http://www.plantnurse.top/appops/newstar.php";
    public static final String GETMYSTAR_URL = "http://www.plantnurse.top/appops/getmystar.php";
    public static final String GETMYPLANT_URL = "http://www.plantnurse.top/myplant/getmyplant.php";
    public static final String ADDPLANT_URL = "http://www.plantnurse.top/myplant/newplant.php";
    public static final String DELETESTAR_URL = "http://www.plantnurse.top/appops/deletestar.php";
    public static final String ADDCOMMENT_URL = "http://www.plantnurse.top/appops/newcomment.php";
    public static final String DELETEPLANT_URL = "http://www.plantnurse.top/myplant/deleteplant.php";
    public static final String FORGETPWD_URL = "http://www.plantnurse.top/appops/forgetpwd.php";
    public static final String CHANGEPLANTPIC_URL = "http://www.plantnurse.top/myplant/changeplantpic.php";
    public static final String CHANGEPLANTINFO_URL = "http://www.plantnurse.top/myplant/changeplantinfo.php";
}
